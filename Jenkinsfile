   ...
   agent any
    ...
    stages {
        ...
        stage('Build Docker Image') {
            steps {
                ...
                bat "docker build -t ${'myapp:14'}:${BUILD_NUMBER} ."
                bat "docker tag ${'myapp:14'}:${BUILD_NUMBER} ${'myapp'}:latest"
            }
        }
        ...
        stage('Push to Registry') {
            steps {
                ...
                withCredentials([usernamePassword(credentialsId: "${'dh2uhf2i'}", usernameVariable: 'USER', passwordVariable: 'PASS')]) {
                    ...
                    bat "docker tag myapp:14 %USER%/myapp:14"
                    bat "docker tag myapp:14 %USER%/myapp:latest"
                    bat "docker push ${'myapp:14'}:${BUILD_NUMBER}"
    ...
    post {
        always {
           ...
           bat script: 'docker rmi myapp:14 myapp:latest dh2uhf2i/myapp:14 dh2uhf2i/myapp:latest', returnStatus: true
        }
        success {
